...
public:
  ...
  int getheight() const;
  void printInfo(int number, std::ostream &output) const;
  int countNodes() const;
  void deleteNode(T data);
  void breadthWalk() const;
  void inOrderWalk() const;
  bool isSimilarTree(const BinarySearchTree &object) const;
private:
  struct Node
  {
  ...
  void search(T data, Node *&ptr) const;
  void countNodes(int &count, Node *ptr) const;
  void searchNext(T data, Node *&ptr) const;
...
template<class T>
int BinarySearchTree<T>::getHeight(Node *&ptr) const
{
  int countLeft = 0;
  int countRight = 0;
...
template<class T>
void BinarySearchTree<T>::printInfo(int number, std::ostream &output) const
{
  if (root_ == nullptr)
  {
...
template<class T>
void BinarySearchTree<T>::countNodes(int &count, Node *ptr) const
{
  count += 1;
  if (ptr->left_ != nullptr)
  {
...
template<class T>
bool BinarySearchTree<T>::isSimilarTree(const BinarySearchTree &object) const
{
  if (countNodes() != object.countNodes())
  {
    return false;
  }
  else
  {
    Node *temp = root_;
    StackList<Node *> stack;
    QueueList<int> result1;
    QueueList<int> result2;
    while (!stack.isEmpty() || temp != nullptr)
    {
      if (temp != nullptr)
      {
    ...
    while (!stack.isEmpty() || temp != nullptr)
    {
      ...
      {
        temp = stack.pop();
        result2.enQueue(temp->key_);
        temp = temp->right_;
      }
    }
    while (!result1.isEmpty())
    {
      if (result1.deQueue() != result2.deQueue())
      {
        return false;
      }
    }
...
template<class T>
void BinarySearchTree<T>::search(T data, Node *&ptr) const
{
  if (root_ == nullptr)
  {
...
template<class T>
void BinarySearchTree<T>::searchNext(T data, Node *&ptr) const
{
  Node *temp = nullptr;
  search(data, temp);
